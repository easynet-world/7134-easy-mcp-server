...
# Step 6: Generate n8n nodes (optional - only if API directory exists)
echo "🔧 Step 6: Generating n8n nodes..."
if [ -d "./api" ] && [ "$(ls -A ./api 2>/dev/null)" ]; then
    if $NPM_PATH run n8n:generate > /dev/null 2>&1; then
        ...
        echo "⚠️  n8n node generation failed or no APIs found. Continuing anyway..."
    fi
else
    echo "ℹ️  No API directory found. Skipping n8n node generation."
    echo "   (This is normal for the main package - n8n nodes are generated in user projects)"
fi
echo ""

# Step 7: Pack the npm package
echo "📦 Step 7: Packing npm package..."
