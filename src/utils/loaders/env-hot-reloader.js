 ...
 * 
 * Watches .env files for changes and reloads them automatically without
 * requiring server restart. Supports multiple .env file variants.
 * 
 * Features:
 * - Watches multiple .env files (.env, .env.local, .env.development)
 * - Automatic reload on file changes
 * - Debouncing to prevent rapid reloads
 * - Integration with MCP server and bridge reloader
 * - API loader notification on changes
 * - Graceful error handling
 * 
 * Watched Files (in priority order):
 * - .env.local (highest priority)
 * - .env.development
 * - .env (lowest priority)
 * 
 * Note: Some environment variable changes may still require server restart
 * depending on when they are read by the application.
 * 
