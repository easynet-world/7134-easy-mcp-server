 ...
 * 
 * Watches for API file changes and automatically reloads routes without
 * requiring server restart. Also handles automatic package installation
 * when missing dependencies are detected.
 * 
 * Features:
 * - File system watching (chokidar-based)
 * - Automatic API reload on file changes
 * - Debouncing to prevent multiple reloads
 * - Automatic package installation (detects and installs missing dependencies)
 * - MCP server notification on route changes
 * - Production mode detection (disabled in production)
 * 
 * Supported Events:
 * - File changes (add, change, unlink)
